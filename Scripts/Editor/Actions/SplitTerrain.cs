...
namespace UnityExtension.Editor.extension.Scripts.Editor.Actions
{
	...
	public class SplitTerrain : EditorWindow {
		...
		void OnGUI()
		{
			parentTerrain = (Terrain)EditorGUILayout.ObjectField("Terrian", parentTerrain, typeof(Terrain), true);
			if (parentTerrain == null)
			{
				EditorGUILayout.HelpBox("Select a terrain from scene first", MessageType.Error);
			}
			terrainsCount = EditorGUILayout.IntField("Count of terrains", terrainsCount);
			var isPowerOfTwo = IsPowerOfTwo(terrainsCount);
			if (!isPowerOfTwo || terrainsCount < 2) 
			{
				EditorGUILayout.HelpBox("Count of result terrains must be a pow of 2 (e. g. 2, 4, 8, 16, ...).", MessageType.Error);
			}
					
			EditorGUILayout.Space(10f, true);
			EditorGUI.BeginDisabledGroup(parentTerrain== null || !isPowerOfTwo || terrainsCount < 2);
